...
set(_OPENVDB_COMPONENT_LIST openvdb openvdb_je pyopenvdb openvdb_ax
                            openvdb_houdini nanovdb)
...
# always make sure openvdb is picked up as a component i.e. find_package(OpenVDB
# COMPONENTS pyopenvdb) results in both openvdb and pyopenvdb targets. Also make
# sure it appears first in the component lists.
list(INSERT OpenVDB_FIND_COMPONENTS 0 openvdb)
list(REMOVE_DUPLICATES OpenVDB_FIND_COMPONENTS)

# Set _OPENVDB_ROOT based on a user provided root var. Xxx_ROOT and
# ENV{Xxx_ROOT} are prioritised over the legacy capitalized XXX_ROOT variables
# for matching CMake 3.12 behaviour @todo  deprecate -D and ENV OPENVDB_ROOT
# from CMake 3.12
if(OpenVDB_ROOT)
...
# This CMake module supports being called from external packages AND from within
# the OpenVDB repository for building openvdb components with the core library
# build disabled. Determine where we are being called from:
#
# (repo structure = <root>/cmake/FindOpenVDB.cmake) (inst structure =
...
elseif(${_DIR_NAME} STREQUAL "OpenVDB")
  ...
  # directory structure is: <root>/lib/cmake/OpenVDB/FindOpenVDB.cmake
  # <root>/include <root>/bin
...
list(APPEND _OPENVDB_INCLUDE_SEARCH_DIRS ${OPENVDB_INCLUDEDIR} ${_OPENVDB_ROOT}
     ${PC_OpenVDB_INCLUDE_DIRS} ${SYSTEM_LIBRARY_PATHS})

foreach(COMPONENT ${OpenVDB_FIND_COMPONENTS})
  # Add in extra component paths
  set(_VDB_COMPONENT_SEARCH_DIRS ${_OPENVDB_INCLUDE_SEARCH_DIRS})
  list(APPEND _VDB_COMPONENT_SEARCH_DIRS ${OPENVDB_${COMPONENT}_ROOT}
       ${OPENVDB_${COMPONENT}_INCLUDEDIR})
  ...
  if(${COMPONENT} STREQUAL "openvdb")
    ...
    find_path(
      OpenVDB_${COMPONENT}_INCLUDE_DIR openvdb/version.h
  ...
  elseif(${COMPONENT} STREQUAL "pyopenvdb")
    find_path(
      OpenVDB_${COMPONENT}_INCLUDE_DIR pyopenvdb.h
      ${_FIND_OPENVDB_ADDITIONAL_OPTIONS}
      PATHS ${_VDB_COMPONENT_SEARCH_DIRS}
      PATH_SUFFIXES
        ${CMAKE_INSTALL_INCLUDEDIR}/openvdb/python
        ${CMAKE_INSTALL_INCLUDEDIR}/openvdb ${CMAKE_INSTALL_INCLUDEDIR} include)
  elseif(${COMPONENT} STREQUAL "openvdb_ax")
    # Look for a standard OpenVDB header file.
    find_path(
      OpenVDB_${COMPONENT}_INCLUDE_DIR compiler/Compiler.h
      ${_FIND_OPENVDB_ADDITIONAL_OPTIONS}
      PATHS ${_VDB_COMPONENT_SEARCH_DIRS}
      PATH_SUFFIXES
        ${CMAKE_INSTALL_INCLUDEDIR}/openvdb/openvdb_ax
        ${CMAKE_INSTALL_INCLUDEDIR}/openvdb_ax ${CMAKE_INSTALL_INCLUDEDIR}
        include)
  elseif(${COMPONENT} STREQUAL "openvdb_houdini")
    # @note  Expects both houdini_utils and openvdb_houdini folders to be
    # located in the same place
    find_path(
      OpenVDB_${COMPONENT}_INCLUDE_DIR openvdb_houdini/SOP_NodeVDB.h
      ${_FIND_OPENVDB_ADDITIONAL_OPTIONS}
      PATHS ${_VDB_COMPONENT_SEARCH_DIRS}
      PATH_SUFFIXES ${CMAKE_INSTALL_INCLUDEDIR}/openvdb
                    ${CMAKE_INSTALL_INCLUDEDIR} include)
  elseif(${COMPONENT} STREQUAL "nanovdb")
    # Look for NanoVDB.h
    find_path(
      OpenVDB_${COMPONENT}_INCLUDE_DIR NanoVDB.h
      ${_FIND_OPENVDB_ADDITIONAL_OPTIONS}
      PATHS ${_VDB_COMPONENT_SEARCH_DIRS}
      PATH_SUFFIXES ${CMAKE_INSTALL_INCLUDEDIR}/nanovdb
                    ${CMAKE_INSTALL_INCLUDEDIR} include)
  endif()
...
openvdb_version_from_header(
  "${_OPENVDB_VERSION_HEADER}"
  VERSION
  OpenVDB_VERSION
  MAJOR
  OpenVDB_MAJOR_VERSION
  MINOR
  OpenVDB_MINOR_VERSION
  PATCH
  OpenVDB_PATCH_VERSION
  ABI
  OpenVDB_ABI_FROM_HEADER # will be OpenVDB_MAJOR_VERSION prior to 8.1.0
)
...
list(APPEND _OPENVDB_LIBRARYDIR_SEARCH_DIRS ${OPENVDB_LIBRARYDIR}
     ${_OPENVDB_ROOT} ${PC_OpenVDB_LIBRARY_DIRS} ${SYSTEM_LIBRARY_PATHS})

# Library suffix handling

set(_OPENVDB_ORIG_CMAKE_FIND_LIBRARY_SUFFIXES ${CMAKE_FIND_LIBRARY_SUFFIXES})

set(OPENVDB_PYTHON_PATH_SUFFIXES lib64/python lib64/python2.7 lib64/python3
                                 lib/python lib/python2.7 lib/python3)
...
foreach(COMPONENT ${OpenVDB_FIND_COMPONENTS})
  ...
  if(${COMPONENT} STREQUAL "pyopenvdb")
    set(_OPENVDB_ORIG_CMAKE_FIND_LIBRARY_PREFIXES
        ${CMAKE_FIND_LIBRARY_PREFIXES})
    set(CMAKE_FIND_LIBRARY_PREFIXES ";lib") # find non-prefixed
    find_library(
      OpenVDB_${COMPONENT}_LIBRARY ${LIB_NAME}
      ${_FIND_OPENVDB_ADDITIONAL_OPTIONS}
      PATHS ${_VDB_COMPONENT_SEARCH_DIRS}
      PATH_SUFFIXES ${OPENVDB_PYTHON_PATH_SUFFIXES})
    set(CMAKE_FIND_LIBRARY_PREFIXES
        ${_OPENVDB_ORIG_CMAKE_FIND_LIBRARY_PREFIXES})
...
# Set the ABI number the library was built against. The old system, which didn't
# define the ABI in the build config, uses vdb_print

if(NOT _OPENVDB_HAS_NEW_VERSION_HEADER)
  if(_OPENVDB_INSTALL)
    openvdb_abi_version_from_print("${_OPENVDB_INSTALL}/bin/vdb_print" ABI
                                   OpenVDB_ABI)
  else()
    # Try and find vdb_print from the include path
    openvdb_abi_version_from_print("${OpenVDB_INCLUDE_DIR}/../bin/vdb_print"
                                   ABI OpenVDB_ABI)
  endif()
endif()
...
# Add deps for pyopenvdb @todo track for numpy

if(pyopenvdb IN_LIST OpenVDB_FIND_COMPONENTS)
  find_package(Python REQUIRED)

  # Boost python handling - try and find both python and pythonXx (version
  # suffixed). Prioritize the version suffixed library, failing if neither
  # exist.

  find_package(Boost ${MINIMUM_BOOST_VERSION} QUIET
  ...
  if(TARGET Boost::python${PYTHON_VERSION_MAJOR}${PYTHON_VERSION_MINOR})
    ...
    message(
      STATUS "Found boost_python${PYTHON_VERSION_MAJOR}${PYTHON_VERSION_MINOR}")
  else()
    find_package(Boost ${MINIMUM_BOOST_VERSION} QUIET COMPONENTS python)
    if(TARGET Boost::python)
      set(BOOST_PYTHON_LIB "python")
      message(
        STATUS "Found non-suffixed boost_python, assuming to be python version "
...
if(openvdb_ax IN_LIST OpenVDB_FIND_COMPONENTS)
  ...
  if(NOT OpenVDB_FIND_QUIET)
    message(
      STATUS
        "Found LLVM: ${LLVM_DIR} (found version \"${LLVM_PACKAGE_VERSION}\")")
  endif()
endif()

# As the way we resolve optional libraries relies on library file names, use the
# configuration options from the main CMakeLists.txt to allow users to manually
# identify the requirements of OpenVDB builds if they know them.
...
if(WIN32)
  ...
  # provided here to maintain backwards compatibility with header include others
...
if(_OPENVDB_HAS_NEW_VERSION_HEADER)
  openvdb_get_version_define(${_OPENVDB_VERSION_HEADER}
                             "OPENVDB_USE_IMATH_HALF" OpenVDB_USES_IMATH_HALF)
  openvdb_get_version_define(${_OPENVDB_VERSION_HEADER} "OPENVDB_USE_BLOSC"
                             OpenVDB_USES_BLOSC)
  openvdb_get_version_define(${_OPENVDB_VERSION_HEADER} "OPENVDB_USE_ZLIB"
                             OpenVDB_USES_ZLIB)
  openvdb_get_version_define(
    ${_OPENVDB_VERSION_HEADER} "OPENVDB_USE_DELAYED_LOADING"
    OpenVDB_USES_DELAYED_LOADING)
elseif(NOT OPENVDB_USE_STATIC_LIBS)
  ...
  get_prerequisites(
    ${OpenVDB_openvdb_LIBRARY} _OPENVDB_PREREQUISITE_LIST
    ${_EXCLUDE_SYSTEM_PREREQUISITES} ${_RECURSE_PREREQUISITES} ""
    "${SYSTEM_LIBRARY_PATHS}")
...
if(NOT TARGET OpenVDB::openvdb)
  ...
  elseif(UNIX)
    ...
    if(_OPENVDB_openvdb_EXT STREQUAL ".a")
      set(OPENVDB_openvdb_LIB_TYPE STATIC)
    elseif(_OPENVDB_openvdb_EXT STREQUAL ".so" OR _OPENVDB_openvdb_EXT STREQUAL
                                                  ".dylib")
  ...
  set_target_properties(
    OpenVDB::openvdb
    PROPERTIES IMPORTED_LOCATION "${OpenVDB_openvdb_LIBRARY}"
  ...
  if(NOT TARGET OpenVDB::pyopenvdb)
    ...
    set_target_properties(
      ...
      PROPERTIES
        ...
        "${OpenVDB_pyopenvdb_INCLUDE_DIR};${PYTHON_INCLUDE_DIR}"
        INTERFACE_LINK_LIBRARIES
        "OpenVDB::openvdb;Boost::${BOOST_PYTHON_LIB};${PYTHON_LIBRARIES}"
        INTERFACE_COMPILE_FEATURES cxx_std_17)
  endif()
endif()
...
if(OpenVDB_openvdb_houdini_LIBRARY)
  if(NOT TARGET OpenVDB::openvdb_houdini)
    ...
    set_target_properties(
      OpenVDB::openvdb_houdini
      PROPERTIES IMPORTED_LOCATION "${OpenVDB_openvdb_houdini_LIBRARY}"
                 INTERFACE_INCLUDE_DIRECTORIES
                 "${OpenVDB_openvdb_houdini_INCLUDE_DIR}"
...
if(OpenVDB_openvdb_ax_LIBRARY)
  ...
  elseif(UNIX)
    ...
    if(_OPENVDB_openvdb_ax_EXT STREQUAL ".a")
      set(OPENVDB_openvdb_ax_LIB_TYPE STATIC)
    elseif(_OPENVDB_openvdb_ax_EXT STREQUAL ".so" OR _OPENVDB_openvdb_ax_EXT
                                                     STREQUAL ".dylib")
  ...
  if(NOT TARGET OpenVDB::openvdb_ax)
    ...
    set_target_properties(
      OpenVDB::openvdb_ax
      PROPERTIES IMPORTED_LOCATION "${OpenVDB_openvdb_ax_LIBRARY}"
                 INTERFACE_INCLUDE_DIRECTORIES
                 "${OpenVDB_openvdb_ax_INCLUDE_DIR}"
...
if(OpenVDB_nanovdb_LIBRARY)
  if(NOT TARGET OpenVDB::nanovdb)
    ...
    set_target_properties(
      OpenVDB::nanovdb
      PROPERTIES IMPORTED_LOCATION "${OpenVDB_nanovdb_LIBRARY}"
